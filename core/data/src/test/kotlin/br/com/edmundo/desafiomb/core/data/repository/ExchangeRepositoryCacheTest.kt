...
class ExchangeRepositoryCacheTest {
    ...
    private suspend fun seedIndexAndExchange(id: Int, syncedAt: Long, updatedAt: Long) {
    ...
    @Test
    fun `dado cache preenchido, quando observeExchanges, entao emite do cache antes de qualquer chamada de rede`() = runTest {
    ...
    @Test
    fun `dado indice expirado, quando loadPage, entao dispara nova chamada ao map`() = runTest {
        ...
        api.onGetExchangeMap = { CmcResponse(CmcStatus(), listOf(ExchangeMapItemDto(id = 1, name = "Exchange 1", slug = "exchange-1"))) }
    ...
    @Test
    fun `dado indice e lista validos, quando loadPage, entao nao dispara nenhuma chamada de rede`() = runTest {
        seedIndexAndExchange(id = 1, syncedAt = timeProvider.currentMillis, updatedAt = timeProvider.currentMillis)
    ...
    @Test
    fun `dado erro de rede no info com cache existente, quando loadPage, entao retorna PageLoad Cached preservando os dados`() = runTest {
    ...
    @Test
    fun `dado erro de rede no info sem nenhum cache existente, quando loadPage, entao retorna failure`() = runTest {
        db.exchangeIndexDao().insertAll(listOf(ExchangeIndexEntity(id = 1, rank = 0, name = "Exchange 1", syncedAt = timeProvider.currentMillis)))
    ...
    @Test
    fun `dado refresh, quando executado, entao invalida e repopula preservando a ordenacao por rank`() = runTest {
        seedIndexAndExchange(id = 1, syncedAt = timeProvider.currentMillis, updatedAt = timeProvider.currentMillis)
    ...
    @Test
    fun `dado erro no map com indice ja existente, quando loadPage, entao degrada servindo o indice antigo`() = runTest {
        val staleSyncedAt = timeProvider.currentMillis - 20.minutes.inWholeMilliseconds
        seedIndexAndExchange(id = 1, syncedAt = staleSyncedAt, updatedAt = timeProvider.currentMillis)
    ...
    @Test
    fun `dado detalhe em cache dentro do TTL, quando syncExchangeDetail, entao nao chama a API`() = runTest {
    ...
    @Test
    fun `dado detalhe expirado, quando syncExchangeDetail, entao busca na API e atualiza o cache`() = runTest {
        ...
        api.onGetExchangeInfo = { CmcResponse(CmcStatus(), mapOf("1" to ExchangeInfoDto(id = 1, name = "Exchange 1", description = "new"))) }
    ...
    @Test
    fun `dado assets nunca sincronizados, quando getExchangeAssets, entao busca na API e persiste`() = runTest {
    ...
    @Test
    fun `dado assets sincronizados e vazios, quando getExchangeAssets novamente dentro do TTL, entao nao chama a API de novo`() = runTest {
    ...
    @Test
    fun `dado header e detalhe em cache, quando observeExchangeDetail, entao combina as duas fontes`() = runTest {
        seedIndexAndExchange(id = 1, syncedAt = timeProvider.currentMillis, updatedAt = timeProvider.currentMillis)
        db.exchangeDetailDao().upsert(
            ExchangeDetailEntity(
                id = 1,
    ...
    @Test
    fun `dado erro de rede no map, quando refresh, entao retorna failure`() = runTest {
        val networkError = IOException("boom")
