...
private class DelayingFakeRepository(
    ...
    override suspend fun loadPage(
        page: Int,
        pageSize: Int,
    ): DomainResult<PageLoad> = throw NotImplementedError("nao utilizado neste teste")
...
class GetExchangeDetailUseCaseTest {
    @Test
    fun `dado info e assets com latencia igual, quando invocado, entao as chamadas rodam em paralelo`() =
        runTest {
            val repository =
                DelayingFakeRepository(
                    detailDelayMs = 100,
                    assetsDelayMs = 100,
    ...
    @Test
    fun `dado detail com falha e assets com sucesso, quando invocado, entao o bundle carrega os dois resultados independentes`() =
        runTest {
            val repository =
                DelayingFakeRepository(
                    detailDelayMs = 0,
    ...
    @Test
    fun `dado detail com sucesso e assets com falha, quando invocado, entao o bundle carrega os dois resultados independentes`() =
        runTest {
            val repository =
                DelayingFakeRepository(
                    detailDelayMs = 0,
