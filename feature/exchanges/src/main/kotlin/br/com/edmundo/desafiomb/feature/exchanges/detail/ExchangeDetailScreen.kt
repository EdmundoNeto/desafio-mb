    ...
    LaunchedEffect(viewModel, lifecycleOwner) {
        lifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
            viewModel.events.collect { event ->
                when (event) {
                    is ExchangeDetailEvent.OpenUrl -> openExternalUrl(context, event.url) {
...
private suspend fun openExternalUrl(context: Context, url: String, onFailure: suspend () -> Unit) {
...
private fun ExchangeDetailContent(
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth().padding(padding)) {
        ...
        when (assetsState) {
            ...
            AssetsState.Empty -> item {
                Text(
                    text = stringResource(R.string.exchange_detail_assets_empty),
                    modifier = Modifier.fillMaxWidth().padding(16.dp).testTag(TestTags.ASSETS_EMPTY),
                )
            }

            is AssetsState.Error -> item {
                Column(modifier = Modifier.fillMaxWidth().padding(16.dp).testTag(TestTags.ASSETS_ERROR)) {
...
@Composable
private fun HeaderSection(header: ExchangeDetailUiModel, onWebsiteClick: () -> Unit) {
    Column(modifier = Modifier.fillMaxWidth().testTag(TestTags.DETAIL_HEADER).padding(16.dp)) {
        Row {
            RemoteImage(
                url = header.logoUrl,
            ...
            Column(modifier = Modifier.padding(start = 16.dp)) {
                ...
                Text(
                    text = stringResource(R.string.exchange_detail_launched_at, header.launchedAt),
        ...
        if (header.websiteUrl != null) {
            Button(
                ...
                modifier = Modifier.padding(top = 8.dp).testTag(TestTags.detailField("website")),
            ) {
...
private fun FeesSection(header: ExchangeDetailUiModel) {
    Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 8.dp)) {
        ...
        Column(modifier = Modifier.padding(start = 32.dp).testTag(TestTags.detailField("taker_fee"))) {
...
private fun AboutSection(description: String?) {
    ...
    Column(modifier = Modifier.fillMaxWidth().padding(16.dp)) {
        ...
        ExpandableText(
            text = description,
            modifier = Modifier.padding(top = 8.dp).testTag(TestTags.detailField("description")),
        )
    }
}
...
private fun AssetsSectionHeader(assetsState: AssetsState) {
    ...
    Text(
        ...
        modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 8.dp).testTag(TestTags.ASSETS_SECTION),
    )
}
