...
fun ExchangeListScreen(
    ...
    Scaffold(
        ...
        when {
            ...
            state.fullScreenError != null -> ErrorState(
            ...
            LazyColumn(state = listState, modifier = Modifier.fillMaxSize().testTag(TestTags.EXCHANGE_LIST)) {
                ...
                item {
                    when (state.appendState) {
                        AppendState.Loading -> Row(
                            modifier = Modifier.fillMaxWidth().padding(16.dp).testTag(TestTags.APPEND_LOADING),
                            horizontalArrangement = Arrangement.Center,
                        ) { CircularProgressIndicator() }

                        is AppendState.Error -> Row(
                            modifier = Modifier
                                .fillMaxWidth()
                                .padding(16.dp)
...
@Composable
private fun ExchangeRow(item: ExchangeUiModel, onExchangeClick: (Int) -> Unit) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Column(modifier = Modifier.padding(start = 16.dp)) {
            ...
            Text(text = "Desde ${item.launchedAt}", style = MaterialTheme.typography.bodySmall)
        }
    }
}
